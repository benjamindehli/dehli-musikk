...
import type { SearchResult as SearchResultData } from "helpers/search";

// Dependencies
import React from "react";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faPhotoFilm, faFilm, faCartShopping, faMusic, faGuitar, faBullhorn, faSliders, faComments } from "@fortawesome/free-solid-svg-icons";
...
const SearchResult = ({ searchResult, lang }: { searchResult: SearchResultData; lang: Lang }) => {
    ...
    const renderThumbnail = (thumbnailPaths: SearchResultData["thumbnailPaths"], alt: string) => {
        return (
            <picture>
        ...
        return (
            <React.Fragment>
                <ListItemThumbnail link={link} compact={true}>
                    {searchResult.thumbnailPaths && searchResult.thumbnailDescription
                        ? renderThumbnail(searchResult.thumbnailPaths, searchResult.thumbnailDescription)
                        : ""}
                </ListItemThumbnail>
                <ListItemContent>
                    <div className={style.searchResultContent}>
                        <div className={style.searchResultContentText}>
                            ...
                            <ListItemContentBody>{searchResult.excerpt}</ListItemContentBody>
                        </div>
                        <div className={`${style.searchResultContentBadge} ${style[searchResult.type]}`}>
                            <span>
                                <FontAwesomeIcon icon={itemTypeIcons[searchResult.type as keyof typeof itemTypeIcons]} /> {searchResult.label}
                            </span>
                        </div>
                    </div>
                </ListItemContent>
