...
import type { Localized, Post as PostData, PostLink } from "types/content";
// Dependencies
...
const Post = ({
    ...
    const renderPostSnippet = (post: PostData, postId: string, postThumbnailSrc: string) => {
        ...
        const snippet: { image: JsonLdImageObject; [key: string]: unknown } = {
            ...
            inLanguage: lang,
            articleBody: post.content[lang] ? formatContentAsString(post.content[lang]) : "",
    ...
    const renderPostThumbnail = (image: ResponsiveImage, altText: string, fullscreen: boolean) => {
        if (fullscreen) {
            return (
                <React.Fragment>
    ...
    const renderLink = (link: PostLink) => {
        return link.internal ? (
            <Link href={`/${languageSlug}${(link.url as Localized)[lang]}`} title={link.text[lang]}>
                <Button buttontype="minimal">{link.text[lang]}</Button>
            </Link>
        ) : (
            <a href={link.url as string} target="_blank" rel="noopener noreferrer" title={link.text[lang]}>
                <Button buttontype="minimal">{link.text[lang]}</Button>
            </a>
        );
    };
    ...
    return post && post.content && post.content[lang] ? (
        <React.Fragment>
            ...
            <ListItemContent fullscreen={fullscreen}>
                ...
                <ListItemContentBody fullscreen={fullscreen}>{formatContentWithReactLinks(post.content[lang], languageSlug)}</ListItemContentBody>
                {post.link && fullscreen ? <ListItemActionButtons fullscreen={fullscreen}>{renderLink(post.link)}</ListItemActionButtons> : ""}
            </ListItemContent>
        </React.Fragment>
    ) : (
        ""
