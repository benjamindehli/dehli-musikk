...
const NavigationBar = () => {
    ...
    const languageSelectorListWrapperRef = useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
        const handleClickOutsideSidebar = (event: MouseEvent) => {
            if (sidebarWrapperRef.current && !sidebarWrapperRef.current.contains(event.target as Node) && showSidebar) {
                hideSidebar();
            }
        };
    ...
    useEffect(() => {
        const handleClickOutsideLanguageSelectorList = (event: MouseEvent) => {
            if (
                languageSelectorListWrapperRef.current &&
                !languageSelectorListWrapperRef.current.contains(event.target as Node) &&
    ...
    const getAlternateLangPath = (targetLang: string) => {
    ...
    const isNavActive = (segment: string) => pathname.includes(`/${segment}/`);
                ...
                <div ref={languageSelectorListWrapperRef} className={`${style.languageSelectorList} ${showLanguageSelectorList ? style.active : ""}`}>
                    <ul>
                        {Object.keys(LANGUAGES).map((langKey) => {
                            const language = LANGUAGES[langKey as Lang];
