...
import type { JsonLdImageObject } from "types/content";
import type { ResponsiveImage } from "types/image";
import type { Video as VideoData } from "types/content";
// Dependencies
...
const Video = ({
    ...
    const renderVideoSnippet = (video: VideoData, videoId: string, videoThumbnailSrc: string) => {
        ...
        const snippet: { thumbnail: JsonLdImageObject; [key: string]: unknown } = {
    ...
    const renderVideoThumbnail = (image: ResponsiveImage, altText: string, compact: boolean) => {
        if (compact) {
            return (
                <React.Fragment>
        ...
        return (
            <React.Fragment>
                ...
                <img {...loadingAttributes} src={image.jpg350 as string} data-width="350" data-height="197" alt={altText} />
            </React.Fragment>
        );
    };
    ...
    const videoDescription = fullscreen ? (
        formatContentWithReactLinks(videoContentString, languageSlug)
    ) : (
        <p>{convertStringToExcerpt(videoContentString)}</p>
    );
    ...
    const theaterModeLabel = isTheaterMode ? (lang === "en" ? "Reduce" : "Forminsk") : lang === "en" ? "Enlarge" : "Forstørr";
        ...
        <React.Fragment>
            {fullscreen ? (
                <React.Fragment>
                    ...
                    <ListItemVideo
                        ...
                        videoTitle={video.title[lang]}
                        thumbnailDescription={video.thumbnailDescription}
            ...
            <ListItemContent fullscreen={fullscreen}>
                <ListItemContentHeader fullscreen={fullscreen} link={link}>
                    ...
                    {!compact && <time dateTime={videoDate.toISOString()}>{getPrettyDate(videoDate, lang)}</time>}
                </ListItemContentHeader>
                {!compact && <ListItemContentBody fullscreen={fullscreen}>{videoDescription}</ListItemContentBody>}
            </ListItemContent>
        </React.Fragment>
    ) : (
        ""
