...
function SearchContent() {
    ...
    const getTitle = () => {
        ...
        return lang === "en" ? `All results with the search term "${searchQuery}"` : `Alle resultater med søkeordet "${searchQuery}"`;
    };

    const heading = getTitle();
    const breadcrumbs = [{ name: heading, path: `/${languageSlug}search/${searchQuery ? `?q=${searchQuery}` : ""}` }];
