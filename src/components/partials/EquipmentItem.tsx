...
const EquipmentItem = ({
    ...
    const renderEquipmentItemImagesSnippet = (images: ResponsiveImage) => {
        const snippet = Object.keys(images).map((format) => {
            const imagePath = images[format as keyof ResponsiveImage];
            return {
     ...
     */
    const renderEquipmentItemSnippet = (item: EquipmentItemData, images: ResponsiveImage, videoCount: number, releaseCount: number) => {
    ...
    const renderPostThumbnail = (image: ResponsiveImage, itemName: string, fullscreen: boolean, compact: boolean) => {
        if (compact) {
            return (
                <React.Fragment>
                    ...
                    <img {...loadingAttributes} src={image.jpg55 as string} data-width="55" data-height="55" alt={itemName} />
                </React.Fragment>
            );
        } else if (fullscreen) {
            return (
                <React.Fragment>
                    ...
                    <img fetchPriority="high" src={image.jpg945 as string} data-width="945" data-height="700" alt={itemName} />
                </React.Fragment>
            );
        } else {
            return (
                <React.Fragment>
                    ...
                    <img {...loadingAttributes} src={image.jpg350 as string} data-width="350" data-height="260" alt={itemName} />
                </React.Fragment>
            );
        }
    };

    const renderVideosList = (videos: VideoData[], lang: Lang, item: EquipmentItemData) => {
    ...
    const renderReleasesList = (releases: LinkedRelease[], lang: Lang, item: EquipmentItemData) => {
        const elementId = `equipment-item-releases-${itemId}`;
