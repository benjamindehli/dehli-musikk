...
export async function generateMetadata({ params }: { params: Promise<{ postId: string }> }): Promise<Metadata> {
    ...
    const title = `${post.title[lang]} - Innlegg | Dehli Musikk`;
    ...
    return {
        ...
        description,
        alternates: {
            canonical: `https://www.dehlimusikk.no/posts/${postId}/`,
            languages: {
...
export default async function PostDetailPage({ params }: { params: Promise<{ postId: string }> }) {
    ...
    const breadcrumbs = [
        { name: 'Innlegg', path: '/posts/' },
        { name: post.title[lang], path: `/posts/${postId}/` }
    ];
