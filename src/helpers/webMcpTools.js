    ...
    return [
        ...
        },
        {
            ...
            async execute({ page }) {
                ...
                if (!response.ok) {
                    return textResult(
                        `No markdown available for ${url.pathname} (HTTP ${response.status}). The search page and error pages are published as HTML only.`
                    );
                }
        ...
        },
        {
            ...
            async execute() {
                ...
                const lines = sectionLinks.map(
                    (section) => `- ${section.label[lang] || section.label.no}: ${WEBSITE_URL}/${languageSlug}${section.path}`
                );
